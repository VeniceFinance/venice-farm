...
contract VeniStaker is ReentrancyGuard, Ownable {
    ...
    IVenice public immutable stakingToken;
    ...
    constructor(
        ...
        stakingToken = IVenice(_stakingToken);
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory) {
        RewardData[] memory rewardsTemp = new RewardData[](rewardTokens.length);
        for (uint256 i = 0; i < rewardsTemp.length; i++) {
            ...
            rewardsTemp[i].token = rewardTokens[i];
            rewardsTemp[i].amount = _earned(account, rewardsTemp[i].token, balance, supply);
        }
        return rewardsTemp;
    }
